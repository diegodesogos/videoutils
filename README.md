...
- **FFmpeg & FFprobe**: Required for video transcoding, metadata extraction, stream mapping, and container manipulations. It natively concatenates and processes DVD `.VOB` files.
...
- **DVD/VOB Directory Support**: Automatically detects DVD structures (containing `VIDEO_TS` or `VIDEO_TS.IFO`). When a DVD directory is detected, it bypasses complex and often broken `.IFO` menus (common in VHS-to-DVD backups) by directly concatenating the sequential `.VOB` files for each Title. It converts these titles seamlessly via `ffmpeg` into independent, optimized `.mp4` files (named `Title_01.mp4`, `Title_02.mp4`, etc.). If the output file exceeds `--maxFileSizeMb`, it is automatically split into smaller chunks (using fast lossless `ffmpeg` segmenting) to facilitate uploads to Google Photos.
